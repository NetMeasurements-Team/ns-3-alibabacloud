...
RdmaEgressQueue::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::RdmaEgressQueue")
            ...
            .AddAttribute ("TokenPerRound", "Token per round in bytes.",
                    UintegerValue(4000),
                    MakeUintegerAccessor (&RdmaEgressQueue::m_token_per_round),
                    MakeUintegerChecker<uint32_t>())
            .AddAttribute ("TxDequeueMode", "The mode of TxDequeue.",
                    EnumValue(TxDequeueMode::DWRR),
                    MakeEnumAccessor(&RdmaEgressQueue::m_txDequeueMode),
                    MakeEnumChecker(TxDequeueMode::DWRR, "DWRR", TxDequeueMode::QP_AVAIL, "QpAvail"))
            ;
    return tid;
}
...
std::pair<Ptr<RdmaQueuePair>, Ptr<Packet>>
RdmaEgressQueue::DoRoundRobin(bool paused[], int& state)
{
    ...
    int res = -1024;
    Ptr<RdmaQueuePair> qplast =
        m_rrlast >= m_qpGrp->m_qps.size() ? nullptr : m_qpGrp->m_qps[m_rrlast];
    if (m_txDequeueMode == TxDequeueMode::DWRR
        && qplast
        && qplast->m_tokenBucket.hasEnoughTokens(qplast->lastPktSize)
        && !paused[qplast->m_pg]
        && qplast->GetBytesLeft() > 0
        && !qplast->IsWinBound())
    {
        // dequeue rrlast
...
void
RdmaEgressQueue::RemoveFinishedQps()
{
...
{
    // if(m_node->GetId() == 0) std::cout << "transmit complete at tick: " <<
...
void
QbbNetDevice::SwitchAsHostTransmitComplete(void)
{
    NS_LOG_FUNCTION(this);
    NS_ASSERT_MSG(m_txMachineState == BUSY, "Must be BUSY if transmitting");
    m_txMachineState = READY;
...
{
    NS_LOG_FUNCTION(this);
    // if(m_node->GetId() == 0) std::cout << "QP start send at tick: " <<
    // Simulator::Now().GetNanoSeconds() << std::endl;
    if (!m_linkUp)
    {
...
void
QbbNetDevice::SwitchAsHostSend(void)
{
    NS_LOG_FUNCTION(this);
    if (!m_linkUp)
    {
    ...
    { // has packet to send
        ...
        {
            Ptr<RdmaQueuePair> lastQp = dequeuePair.first;
            NS_ASSERT_MSG(lastQp->nvls_enable == 1 && m_node->GetNodeType() == 2,
    ...
    { // no packet to send
        ...
        for (uint32_t i = 0; i < fcount; i++)
        {
            ...
            {
                Time newt = qp->m_rate.CalculateBytesTxTime((int64_t)m_rdmaEQ->m_token_per_round -
                                                            qp->m_tokenBucket.m_tokens);
                newt = newt.GetTimeStep() == 0 ? NanoSeconds(1) : newt;
                t = Min(newt + Simulator::Now(), t);
            }
        }
        ...
        {
            m_nextSend =
                Simulator::Schedule(t - Simulator::Now(), &QbbNetDevice::SwitchAsHostSend, this);
        }
    }
}

void
QbbNetDevice::SwitchDequeueAndTransmit(void)
{
    NS_LOG_FUNCTION(this);
    // if(m_node->GetId() == 0) std::cout << "QP start send at tick: " <<
    // Simulator::Now().GetNanoSeconds() << std::endl;
...
QbbNetDevice::Receive(Ptr<Packet> packet)
{
    ...
    { // non-PFC packets (data, ACK, NACK, CNP...)
        ...
        if (m_node->GetNodeType() > 0 && ch.m_tos != 4 && did != m_node->GetId())
        { // switch
            // std::cout << "id: " << m_node->GetId() << " switch receive from " << sid <<
            // std::endl;
...
Ptr<Packet>
QbbNetDevice::NICSendPfc(uint32_t qIndex, uint32_t type)
{
    Ptr<Packet> p = Create<Packet>(0);
    PauseHeader pauseh((type == 0 ? m_pausetime : 0), m_queue->GetNBytes(qIndex), qIndex);
...
void
QbbNetDevice::SendCallback(Ptr<Packet> packet)
{
    CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
    packet->PeekHeader(ch);
...
bool
QbbNetDevice::SwitchAsHostTransmitStart(Ptr<Packet> p)
{
    NS_LOG_FUNCTION(this << p);
    NS_LOG_LOGIC("UID is " << p->GetUid() << ")");
    //
