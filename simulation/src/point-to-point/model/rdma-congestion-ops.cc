...
TypeId
RdmaCongestionOps::GetTypeId(void)
{
...
void
RdmaCongestionOps::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> p)
{
    // do nothing
}

void
RdmaCongestionOps::QpComplete(Ptr<RdmaQueuePair> qp)
{
    // do nothing
}
...
void
RdmaCongestionOps::ChangeRate(DataRate new_rate)
{
...
void
MellanoxDcqcn::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
...
void
MellanoxDcqcn::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    ...
    if (cnp)
    {
        ...
        if (m_first_cnp)
        {
            ...
            ScheduleDecreaseRate(qp,
                                 1); // add 1 ns to make sure rate decrease is after alpha update
...
void
MellanoxDcqcn::UpdateAlpha(Ptr<RdmaQueuePair> q)
{
#if PRINT_LOG
...
void
MellanoxDcqcn::CheckRateDecrease(Ptr<RdmaQueuePair> q)
{
    ...
    if (m_decrease_cnp_arrived)
    {
        ...
        m_rpTimer = Simulator::Schedule(MicroSeconds(m_rpgTimeReset),
                                        ...
                                        q);
#if PRINT_LOG
        printf("(%.3lf %.3lf)\n", m_targetRate.GetBitRate() * 1e-9, m_ccRate.GetBitRate() * 1e-9);
#endif
    }
}
...
void
MellanoxDcqcn::RateIncEvent(Ptr<RdmaQueuePair> q)
{
...
void
MellanoxDcqcn::FastRecovery(Ptr<RdmaQueuePair> q)
{
#if PRINT_LOG
...
void
MellanoxDcqcn::ActiveIncrease(Ptr<RdmaQueuePair> q)
{
#if PRINT_LOG
...
void
MellanoxDcqcn::HyperIncrease(Ptr<RdmaQueuePair> q)
{
#if PRINT_LOG
...
void
Hpcc::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
...
void
Hpcc::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint64_t ack_seq = ch.ack.seq;
    // update rate
    if (ack_seq > m_lastUpdateSeq)
...
void
Hpcc::UpdateRate(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch, bool fast_react)
{
        ...
        if (ih.nhop <= IntHeader::maxHop)
        {
            ...
            for (uint32_t i = 0; i < ih.nhop; i++)
            {
                ...
                else
                {
                    ...
                    hopState[i].u =
                        (hopState[i].u * (qp->m_baseRtt - tau) + u * tau) / double(qp->m_baseRtt);
                }
                hop[i] = ih.hop[i];
            }
...
void
Hpcc::FastReact(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    if (m_fast_react)
    {
...
void
Timely::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
    m_curRate = lazyInitRate;
}
...
void
Timely::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint64_t ack_seq = ch.ack.seq;
    // update rate
    if (ack_seq > m_lastUpdateSeq)
...
void
Timely::UpdateRate(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch, bool us)
{
    uint64_t next_seq = qp->snd_nxt;
    ...
    { // not first RTT
        ...
        double rtt_diff = (1 - m_tmly_alpha) * m_rttDiff + m_tmly_alpha * new_rtt_diff;
...
void
Timely::FastReact(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    // do nothing
}
...
void
Dctcp::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint64_t ack_seq = ch.ack.seq;
    uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
...
TypeId
HpccPint::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::HpccPint")
                            .SetParent<Hpcc>()
                            .AddConstructor<HpccPint>()
                            .AddAttribute("PintProb",
                                          "PINT's probability, using to calculate PintSmplThresh, "
                                          "which is PINT's sampling threshold in rand()%65536",
                                          DoubleValue(1.0),
...
void
HpccPint::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
    m_curRate = lazyInitRate;
}
...
void
HpccPint::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint64_t ack_seq = ch.ack.seq;
...
void
HpccPint::UpdateRate(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch, bool fast_react)
{
    uint64_t next_seq = qp->snd_nxt;
    if (m_lastUpdateSeq == 0)
    { // first RTT
...
void
Swift::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
    if (m_gamma == 0)
    {
        m_gamma = 1. / (1. / std::sqrt(rdmaHw->m_minRate.GetBitRate()) -
                        1. / std::sqrt(qp->m_max_rate.GetBitRate())); // 1/(1/√minrate - 1/√maxrate)
    }
    m_curRate = lazyInitRate;
}

void
Swift::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint64_t rtt = Simulator::Now().GetTimeStep() - ch.ack.ih.ts;
    uint64_t target = qp->m_baseRtt;
    double range =
        m_range * m_gamma *
        (1. / std::sqrt(m_curRate.GetBitRate()) - 1. / std::sqrt(qp->m_max_rate.GetBitRate()));
    if (range > m_range)
    {
        range = m_range;
    }
    ...
    else if (m_canDecrease)
    {
        ...
        m_eventSetCanDecrease =
            Simulator::Schedule(NanoSeconds(rtt), &Swift::SetCanDecreaseToTrue, this);
    ...
    static TypeId tid =
        TypeId("ns3::RealDcqcn")
            ...
            .AddAttribute("Clamp",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("ClampAfterTimeInc",
...
void
RealDcqcn::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
    m_targetRate = lazyInitRate;
...
void
RealDcqcn::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> p)
{
...
void
RealDcqcn::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
    if (cnp)
    {
...
void
HpccEcn::LazyInit(Ptr<RdmaQueuePair> qp, DataRate lazyInitRate)
{
    RdmaCongestionOps::LazyInit(qp, lazyInitRate);
    m_maxRate = lazyInitRate;
    DoInit();
...
void
HpccEcn::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> p)
{
...
void
HpccEcn::HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
{
    uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
    if (cnp)
    {
        ...
        m_rampUpStage = 0;
        if (m_cnpCounter == 1)
        { // reduce rate only once in a loop
